...
/* ── Software ──
   ...
   views/software/, so the whitelist is read from that directory at boot rather
   than typed out here — a list copied from the generator would drift the first
   time a trade is added, and a 404 on a page that exists is the kind of bug
   nobody notices for a month. An unknown slug still falls through to the 404
   handler, because only names that are actually files can match. */
const SOFTWARE_DIR   = path.join(VIEWS_DIR, 'software');
const SOFTWARE_PAGES = new Set(
  require('fs').readdirSync(SOFTWARE_DIR)
    .filter(f => f.endsWith('.html') && f !== 'index.html')
    .map(f => f.replace(/\.html$/, ''))
);

app.get('/software', (req, res) =>
  res.sendFile(path.join(SOFTWARE_DIR, 'index.html')));

app.get('/software/:slug', (req, res, next) => {
  if (!SOFTWARE_PAGES.has(req.params.slug)) return next();
  res.sendFile(path.join(SOFTWARE_DIR, `${req.params.slug}.html`));
});
